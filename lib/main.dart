...
Future<void> main() async {
  //aqui es el problema de la pantalla negra
  WidgetsFlutterBinding
      .ensureInitialized(); //se asegura que se realize de forma asincrona (Firebase)
  await Firebase
      .initializeApp(); //tenemos que esperar los servicios de firebase (Firebase)
  runApp(const MyApp());
}
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      //se encarga de checar los servicios
      providers: [
        ...
        StreamProvider(
            create: (context) => context
                .read<AuthentificationFirebase>()
                .authStateChanges, //se encargara de checar el estado del usuario
            initialData: null),
      ],
      child: MaterialApp(
        ...
        home:
            const ChecarAutentificacion(), //vamos a cambiar la pagina de inicio segun el estado del login (Firebase)
      ),
    );
  }
}
...
class ChecarAutentificacion extends StatelessWidget {
  const ChecarAutentificacion({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final firebaseUser = context
        .watch<User?>(); //checamos el estado del usuario checar error aqui
    if (firebaseUser != null) {
      //return Text('signed in');
      return const ClienteScreen2();
